...
void ttz_plot()
{
  ...
  vector<string>  nj_reg={"0","1"}; //,"1","2","3","4"
  vector<string> variable={"lep_Pt_0","lep_Pt_1","lep_Pt_2","min_DRl0j","maxEta_ll","HT_jets","HT","nJets","nBtagJets","MET"}; //HT",
  vector<string> type={"sherpa_413022","Sherpa_413023"};
  ...
  string base_name="input/Res_TTZ";  
  string file_name;
  ...
  for(int i=0;i<nj_reg.size();i++){
	...
	//}    
      ...
      sprintf(text1,"#sqrt{s} = 13 TeV, 3l %s ",region_names[i].c_str());
	...
	h_var[i][j][3][t]->Draw("histsame");
      ...
      sprintf(o_name,"Plots_TTZ_v0/%s.pdf",canvas_name);
